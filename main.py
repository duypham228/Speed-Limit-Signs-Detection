...
# image = "46-30.jpeg"
    ...
    print(image)
    if (image == ".DS_Store"): #because macs are dumb
        continue
    ...
    resize_img = cv2.resize(gray, dsize)
    ...
    blackWhite = numpy.array(list(map(mapping_helper, crop)), dtype="uint8")
    ...
    for i in range(0, len(blackWhite[0]), portion):
        ...
        # print(i, blackRatio)
        if i > portion * 3.5 and i < portion * 9: #increase the threshold to ignore sides
    ...
    split_images_path = os.path.join(dirname, "dev_ws/backup-images")
